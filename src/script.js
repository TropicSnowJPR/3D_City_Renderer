...
async function init() {
    ...
    DOWNLOAD.add( GUI_PARAMS.Download, 'exportGLB' );
    ...
    const BASEPLATE = new THREE.Mesh(new THREE.CylinderGeometry( RADIUS, RADIUS, 5, 128, 1,), BASEPLATE_MATERIAL );
    BASEPLATE.position.set(0, -2.5, 0);
    BASEPLATE.receiveShadow = true;
    SCENE.add( BASEPLATE );

    const BASEPLATE_EDGE_MATERIAL = new THREE.MeshStandardMaterial({ color: 0xa0a0a0 });
    const BASEPLATE_EDGE = new THREE.Mesh(new THREE.CylinderGeometry( RADIUS+3, RADIUS+3, 6, 128, 1, ), BASEPLATE_EDGE_MATERIAL );
